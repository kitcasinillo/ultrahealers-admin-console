...
// Historical data will be fetched from the backend stats
...
export function Payments() {
    ...
    useEffect(() => {
        const fetchStats = async () => {
            try {
                ...
                const response = await fetch(`${apiUrl}/api/admin/finance/revenue-stats`);
    ...
    const chartData = stats?.distribution ? [
        { name: 'Sessions', value: stats.distribution.sessionCommissions },
        { name: 'Premium', value: stats.distribution.premiumSubscriptions },
        { name: 'Retreats', value: stats.distribution.retreatPlatformFees },
        { name: 'Others', value: stats.distribution.others },
    ] : [];
    ...
    return (
        <Tabs.Root value={activeTab} onValueChange={setActiveTab} className="space-y-6 block" onClick={() => setIsDropdownOpen(false)}>
            {/* Header section with unified controls */}
            <div className="flex flex-col gap-6">
                <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
                    <div>
                        <h2 className="text-4xl font-black tracking-tight text-[#1b254b] dark:text-white">Payments Center</h2>
                        <p className="text-[#A3AED0] text-base mt-2 font-medium">
                            Comprehensive revenue tracking and platform financial management.
                        </p>
                    </div>
                </div>

                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white/50 dark:bg-[#111C44]/50 p-2 rounded-[24px] border border-[#E9EDF7] dark:border-white/5 backdrop-blur-xl">
                    <Tabs.List className="flex gap-1">
                        <Tabs.Trigger
                            value="overview"
                            className={`px-8 py-3 rounded-2xl text-sm font-black transition-all ${activeTab === 'overview'
                                    ? "bg-white dark:bg-[#1b254b] text-[#4318FF] dark:text-white shadow-[0_4px_12px_rgba(0,0,0,0.05)]"
                                    : "text-[#A3AED0] hover:text-[#1b254b] dark:hover:text-white hover:bg-white/50 dark:hover:bg-white/5"
                                }`}
                        >
                            Revenue Overview
                        </Tabs.Trigger>
                        <Tabs.Trigger
                            value="commission"
                            className={`px-8 py-3 rounded-2xl text-sm font-black transition-all ${activeTab === 'commission'
                                    ? "bg-white dark:bg-[#1b254b] text-[#4318FF] dark:text-white shadow-[0_4px_12px_rgba(0,0,0,0.05)]"
                                    : "text-[#A3AED0] hover:text-[#1b254b] dark:hover:text-white hover:bg-white/50 dark:hover:bg-white/5"
                                }`}
                        >
                    ...
                    {activeTab === 'overview' && (
                        <div className="relative px-2">
                            <button
                                ...
                                className="flex items-center gap-3 bg-white dark:bg-[#111C44] px-6 py-3 rounded-2xl border border-[#E9EDF7] dark:border-white/5 shadow-sm transition-all hover:border-[#4318FF] dark:hover:border-[#01A3B4] group"
                            >
                            ...
                            {isDropdownOpen && (
                                <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-[#1b254b] rounded-2xl shadow-[0_20px_40px_0_rgba(11,20,55,0.15)] border border-[#E9EDF7] dark:border-white/5 py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                                    {ranges.map((range) => (
                                        <button
                                            key={range.id}
                                            onClick={() => {
                                            ...
                                            className={`w-full text-left px-5 py-2.5 text-sm font-bold transition-colors ${timeRange === range.id
            ...
            <Tabs.Content value="overview" className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500">

                {/* Quick Stats Grid */}
                <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-4">
                    <StatsCard
                        title="Total Platform Revenue"
                        value={loading ? "..." : formatCurrency(stats?.totalPlatformRevenue || 0)}
                    ...
                    <StatsCard
                        ...
                        value={loading ? "..." : formatCurrency(stats?.netPlatformRevenue || 0)}
                    ...
                    <StatsCard
                        ...
                        value={loading ? "..." : formatCurrency(stats?.totalGrossBookingVolume || 0)}
                    ...
                    <StatsCard
                        ...
                        value={loading ? "..." : formatCurrency(stats?.totalPremiumRevenue || 0)}
                    ...
                    <div className="lg:col-span-8 bg-white dark:bg-[#111C44] rounded-[30px] p-6 shadow-[0_10px_30px_0_rgba(11,20,55,0.06)] dark:shadow-none border border-transparent dark:border-white/5">
                        ...
                        <div className="h-[350px] w-full mt-4">
                            <ResponsiveContainer width="100%" height="100%">
                                <AreaChart
                                    data={stats?.chartData || FALLBACK_CHART_DATA}
                            ...
                            {chartData.map((item, index) => (
                                <div key={item.name} className="flex items-center justify-between">
                                    ...
                                    <span className="text-sm font-bold text-[#1b254b] dark:text-white">
                                        {((item.value / (stats?.totalPlatformRevenue || 1)) * 100).toFixed(1)}%
                                    </span>
                                </div>
                            ))}
                        </div>
            ...
            <Tabs.Content value="commission" className="animate-in fade-in duration-500">
                <CommissionTable />
            </Tabs.Content>
        </Tabs.Root>
    );
