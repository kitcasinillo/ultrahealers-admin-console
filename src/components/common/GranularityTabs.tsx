...
export function GranularityTabs({
  ...
  return (
    <div className="flex bg-[#F4F7FE] dark:bg-white/5 p-1.5 rounded-full shadow-inner border border-gray-50/50 dark:border-white/5 w-full sm:w-auto">
      {options.map((item) => (
        <button
          key={item}
          onClick={() => setGranularity(item)}
          className={cn(
            "flex-1 sm:flex-none text-center px-6 py-2 rounded-full text-sm font-bold transition-all whitespace-nowrap cursor-pointer",
            granularity === item
