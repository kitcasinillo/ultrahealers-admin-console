...
- [x] **0.2 Install Dependencies:** Add Tailwind CSS v3, Radix UI / shadcn/ui, Recharts, TanStack Table v8, React Hook Form, Zod, and Firebase SDK.
- [x] **0.3 Architecture Setup:** Create folder structure (`src/pages`, `src/components`, `src/lib`, `src/contexts`).
...
- [ ] **2.3 Module 5 & 6 — Bookings Management:**
  - [x] Build `/bookings/sessions` data table with status flags and filters.
  - [x] Build Session Booking Detail Page (commission breakdown, chat viewer).
...
## 💎 Phase 9: Premium UI & Refinement (Senior Refactor)
- [ ] **9.1 Architecture Audit:**
  - [ ] Move `useBookings` destructive hook logic out of the table cell renderer to top-level.
  - [ ] Rename `cancelBooking` to `initialCancelBooking` to resolve naming conflicts and ensure clean state updates.
- [ ] **9.2 Horizon UI Aesthetic Alignment:**
  - [ ] Apply Horizon UI branding: `--primary: 250 84% 54%` (Brand Blue), `#A3AED0` (Secondary Gray), and rounded card styles.
  - [ ] Use glassmorphism for dropdown menus and absolute-positioned elements.
- [ ] **9.3 Proper Confirmation Modal:**
  - [ ] Implement `src/components/ui/alert-dialog.tsx` (Shadcn/UI + Radix).
  - [ ] Replace `window.confirm` with a custom modal that matches the Horizon UI theme.
- [ ] **9.4 Final Column Order (as per 5.1 docs):**
  - [ ] **Order:** Booking ID | Listing | Healer | Seeker | Session Date | Amount | Currency | Commission | Status Flags | Payment Status | Created | Actions.
- [ ] **9.5 Cleanup:**
  - [ ] Remove all `window.alert` and `window.confirm` browser defaults.
  - [ ] Add loading skeletons for a smooth "Skeleton Screen" experience during fetching.
